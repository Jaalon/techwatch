...
NOTE: You can override the backend HTTP port via `backend/techwatch.properties` (property `quarkus.http.port`).
...
- Swagger UI: http://localhost:8080/q/swagger-ui (if enabled)

- AI API Keys endpoints (credentials store):
* `GET /api/ai-keys` — list stored keys (secrets masked via `keyPreview`)
* `GET /api/ai-keys/{id}` — fetch one key with full `apiKey` (for edit UI only)
* `POST /api/ai-keys` — create `{ provider, name, baseUrl, apiKey, organizationId?, projectId? }`
